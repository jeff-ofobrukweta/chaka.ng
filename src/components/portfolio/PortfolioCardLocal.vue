<template>
    <Fragment>
    <div v-if="instrument" 
        class="portfolio-card" @click="toDetailsPage('local')">
            <div class="portfolio-card__img">
                <img :src="require(`../../assets/img/portfolio1.svg`)" alt="Portfolio Icon" />
            </div>
            <h2 class="cursor-context" :title="instrument.positions.filled.localOrders | currency('NGN', true)">
                {{ instrument.positions.filled.localOrders | currency('NGN') }}
            </h2>
            <p class="portfolio-card__title">{{ 'Total Value on Local Stocks' }}</p>
            <table v-if="(instrument.positions.filled.local.length > 0) && (instrument.positions.filled)" class="portfolio-card__table">
