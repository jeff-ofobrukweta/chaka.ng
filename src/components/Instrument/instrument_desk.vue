            ...
            <router-link :to="{name:'singlestock',params:{symbol:item.symbol}}">
                <section class="section2">
                    ...
                    <div class="level">
                        ...
                        <span
                        :class="[item.derivedPrice >= 0 ? 'green' : 'red']"
                        >{{item.derivedPrice | unit(2)}}({{item.derivedPricePercentage | unit(2)}})</span></div>
                     <img :src="require(`../../assets/img/flags/${item.countryCode? item.countryCode.toLowerCase(): 'zz'}-flag.svg`)" 
