...
const actions = {
    GET_KYC: ({ commit, rootState }) => new Promise(resolve => api.get(`/users/${rootState.auth.loggedUser.chakaID}/kyc`).then(
        (resp) => {
    ...
    GET_NAVBAR_NEXT_KYC: ({ commit, rootState }) => new Promise(resolve => api.get(`/users/${rootState.auth.loggedUser.chakaID}/fetch-next-kyc`).then(
        (resp) => {
            if (resp.status >= 200 && resp.status < 400) {
                commit('SET_NAVBAR_NEXT_KYC', resp.data.data);
                EventBus.$emit('navbar-trigger');
    ...
    UPDATE_KYC: ({ commit, dispatch, rootState }, payload) => {
        ...
        return new Promise(resolve => api.patch(`/users/${rootState.auth.loggedUser.chakaID}/kyc`, params).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
    ...
    UPDATE_KYC_NIN: ({ commit, dispatch, rootState }, payload) => {
        ...
        commit('REQ_INIT', null, { root: true });
        const { source, ...params } = payload;
        return new Promise(resolve => api.post(`/users/${rootState.auth.loggedUser.chakaID}/kyc/update-nin`, params).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    commit('SET_KYC', resp.data.data.kyc);
    ...
    UPLOAD_KYC_FILE: ({ commit, dispatch, rootState }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.post(`/users/${rootState.auth.loggedUser.chakaID}/kyc/upload-file`, payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    dispatch('GET_LOGGED_USER', null, { root: true });
    ...
    RESOLVE_BVN: ({ commit, dispatch, rootState }, payload) => {
        ...
        commit('REQ_INIT', null, { root: true });
        const { source, ...params } = payload;
        return new Promise(resolve => api.post(`/users/${rootState.auth.loggedUser.chakaID}/kyc/resolve-bvn`, params).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    commit('SET_KYC', resp.data.data.kyc);
    ...
    USE_BVN_PHONE: ({ commit, dispatch, rootState }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.post(`/users/${rootState.auth.loggedUser.chakaID}/kyc/update-phone`, payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    dispatch('GET_KYC');
    ...
    RESOLVE_OTP: ({ commit, dispatch, rootState }, payload) => {
        ...
        commit('REQ_INIT', null, { root: true });
        const { source, ...params } = payload;
        return new Promise(resolve => api.post(`/users/${rootState.auth.loggedUser.chakaID}/kyc/resolve-otp`, params).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    dispatch('GET_KYC');
