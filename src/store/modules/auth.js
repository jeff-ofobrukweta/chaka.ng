...
const actions = {
    REGISTER: ({ commit }, payload) => {
        ...
        return new Promise(resolve => api.post('/auth/signup', payload).then(
            (resp) => {
    ...
    FORGOT_PASSWORD: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.post('/auth/reset-password/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    RESET_PASSWORD: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.post('/auth/reset-password/confirm/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    CONFIRM_EMAIL: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.post('/auth/confirm-email-token/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    LOGIN: ({ commit }, payload) => {
        ...
        commit('REQ_INIT', null, { root: true });
        mixpanel.identify(payload.email); // In sign up
        return new Promise(resolve => api.post('/auth/login', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
    ...
    SOCIAL_LOGIN: ({ commit }, payload) => {
        ...
        commit('REQ_INIT', null, { root: true });
        mixpanel.identify(payload.email); // In sign up
        return new Promise(resolve => api.post('/auth/social-auth', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    localStorage.setItem('AUTH_TOKEN', resp.data.data.token);
                    localStorage.setItem('REFRESH_TOKEN', resp.data.data.refreshToken);
    ...
    GET_LOGGED_USER: ({ commit }) => {
        ...
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise(resolve => api.get(`/users/${token.user.chakaID}`).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    commit('REQ_SUCCESS', null, { root: true });
