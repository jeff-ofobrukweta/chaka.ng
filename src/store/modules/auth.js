...
const actions = {
    REGISTER: ({ commit }, payload) => {
        ...
        return new Promise((resolve, reject) => api.post('/auth/signup', payload).then(
            (resp) => {
    ...
    FORGOT_PASSWORD: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise((resolve, reject) => api.post('/auth/reset-password/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    RESET_PASSWORD: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise((resolve, reject) => api.post('/auth/reset-password/confirm/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    CONFIRM_EMAIL: ({ commit }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise((resolve, reject) => api.post('/auth/confirm-email-token/', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    resolve(true);
    ...
    LOGIN: ({ commit, dispatch }, payload) => {
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise((resolve, reject) => api.post('/auth/login', payload).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
    ...
    GET_LOGGED_USER: ({ commit }) => {
        ...
        commit('RESET_REQ', null, { root: true });
        commit('REQ_INIT', null, { root: true });
        return new Promise((resolve, reject) => api.get(`/users/${token.user.chakaID}`).then(
            (resp) => {
                if (resp.status >= 200 && resp.status < 400) {
                    commit('REQ_SUCCESS', null, { root: true });
                    commit('REQ_SUCCESS', null, { root: true });
