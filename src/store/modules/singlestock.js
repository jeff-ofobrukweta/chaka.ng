...
const actions = {
    async GET_SINGLESTOCK_INSTRUMENT({ commit, dispatch }, params) {
        return new Promise(resolve => API_CONTEXT.get(`/instruments/?symbols=${params.symbols}`)
            .then((response) => {
    ...
    GET_SIMILAR_STOCKS({ commit }, params) {
        return new Promise(resolve => API_CONTEXT.get(`/instruments/?symbols=${params.join(',')}`).then((response) => {
            if (response.status === 200) {
                const { instruments } = response.data.data;
    ...
    GET_OPEN_ORDERS({ commit, rootState }) {
        return new Promise(resolve => API_CONTEXT.get(`/users/${rootState.auth.loggedUser.chakaID}/orders/open/`).then(
            (response) => {
                if (response.status === 200) {
    ...
    CANCEL_ORDER: ({ commit, dispatch, rootState }, payload) => {
        ...
        return new Promise(resolve => API_CONTEXT.post(
    ...
    GET_PRE_ORDER: ({ commit, rootState }, payload) => new Promise(resolve => API_CONTEXT.post(
    ...
    GET_MARKET_DATA: ({ commit }, payload) => new Promise(resolve => API_CONTEXT.get(`/instruments/market-data?symbol=${payload}`).then(
        (resp) => {
            if (resp.status >= 200 && resp.status < 400) {
    ...
    SEARCH_INSTRUMENTS: ({ commit }, payload) => new Promise(resolve => API_CONTEXT.get('/search', { ...payload }).then(
        (resp) => {
            if (resp.status >= 200 && resp.status < 400) {
